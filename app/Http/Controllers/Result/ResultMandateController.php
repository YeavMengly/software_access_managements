...
class ResultMandateController extends Controller
{
    ...
    private function applyDateFilter($query, $startDate, $endDate)
    {
        if ($startDate && $endDate) {
            try {
                ...
                $reports = Mandate::whereBetween('created_at', [$startDate, $endDate])->get(['report_key']);
            ...
            try {
                $startDate = Carbon::createFromFormat('Y-m-d', $startDate)->startOfDay()->toDateTimeString();
                $reports = Mandate::where('created_at', '>=', $startDate)->get(['report_key']);
                $reports = !empty($reports) ? $reports->toArray() : [];
    ...
    private function calculateTotals($dataMandates)
    {
        $totals = [
            'fin_law' => 0,
            'current_loan' => 0,
        ...
        foreach ($dataMandates as $dataMandate) {
            ...
            if (!isset($totals['reportKey'][$codeId][$accountKeyId][$subAccountKeyId][$reportKeyId])) {
                $totals['reportKey'][$codeId][$accountKeyId][$subAccountKeyId][$reportKeyId] = [
                    'name_report_key' => $dataMandate->name_report_key,
                    'fin_law' => 0,
                    'current_loan' => 0,
                    'internal_increase' => 0,
                    'unexpected_increase' => 0,
        ...
        $groupedByCode = $dataMandates->groupBy(function ($loan) {
    ...
    private function calculateSumFields($dataMandates)
    {
        $sumFields = [
            'fin_law' => 0,
            'current_loan' => 0,
