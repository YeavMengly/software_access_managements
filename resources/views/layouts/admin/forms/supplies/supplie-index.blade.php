        ...
        <div class="result-total-table-container">
            <div class="row">
                <div class="col-lg-12 margin-tb">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="btn-group">
                            <a href="" data-bs-toggle="modal" data-bs-target="#createFuel"
                                ...
                                data-bs-toggle="tooltip" title="បង្ហាញលទ្ធផល">
                    ...
                    <div class="modal fade" id="createFuel" tabindex="-1" aria-labelledby="importModalLabel"
                        aria-hidden="true">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12 d-flex">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}"
                                    class="form-control mb-2" placeholder="ស្វែងរកតាមរយៈ​ លេខប័ណ្ណបញ្ចូល នឹងបញ្ចេញ បរិយាយ"
                        ...
                        <tr>
                            ...
                            @foreach ($fuelTags as $fuelTag)
                                @if ($fuelTag->fuel_tag === $oilType)
                                    <td style="border: 1px solid black;">{{ number_format($fuel->quantity, 0) }}</td>
                                    <td style="border: 1px solid black;">{{ number_format($fuel->quantity_used, 0) }}
                                    </td>
            ...
            {{-- <div class="d-flex justify-content-between align-items-center mt-4">
                <div>
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            <li class="page-item{{ $fuels->onFirstPage() ? ' disabled' : '' }}">
                                <a class="page-link"
                                    href="{{ $fuels->previousPageUrl() }}&per_page={{ request('per_page') }}"
                                    aria-label="Previous">
                            ...
                            @for ($i = 1; $i <= $fuels->lastPage(); $i++)
                                <li class="page-item{{ $fuels->currentPage() == $i ? ' active' : '' }}">
                                    <a class="page-link"
                                        href="{{ $fuels->url($i) }}&per_page={{ request('per_page') }}">{{ $i }}</a>
                                </li>
                            @endfor
                            <li class="page-item{{ !$fuels->hasMorePages() ? ' disabled' : '' }}">
                                <a class="page-link"
                                    href="{{ $fuels->nextPageUrl() }}&per_page={{ request('per_page') }}"
                                    aria-label="Next">
