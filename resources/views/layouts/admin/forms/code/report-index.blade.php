        ...
        <tbody>
            @forelse ($reports as $report)
                <tr>
                    ...
                    <td style="border: 1px solid black; max-width: 80px; text-align: center">{{ $report->fin_law }}</td>
                    <td style="border: 1px solid black; max-width: 80px; text-align: center">{{ $report->current_loan }}
                    </td>
                    <td style="border: 1px solid black; max-width: 80px; text-align: center">
                        {{ $report->internal_increase }}</td>
                    <td style="border: 1px solid black; max-width: 80px; text-align: center">
                        {{ $report->unexpected_increase }}</td>
                    <td style="border: 1px solid black; max-width: 80px; text-align: center">
                        {{ $report->additional_increase }}</td>
                    <td style="border: 1px solid black; max-width: 80px; text-align: center">{{ $report->decrease }}</td>
                    <td
                        style="border: 1px solid black; max-width: 90px; text-align: center; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                        <div class="form-container">
                            <form action="{{ route('codes.destroy', $report->id) }}" method="POST">
                                <a class="btn btn-primary" href="{{ route('codes.edit', $report->id) }}">Edit</a>
                                @csrf
                                @method('DELETE')
