...
@section('content-usage-units')
    <div class="border-wrapper">
        <div class="result-total-table-container">
            <div class="row">
                <div class="col-lg-12 margin-tb mb-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <h3 class="mx-auto" style="font-weight: 500;">តារាងអង្គភាប្រើប្រាស់អគ្គិសនី</h3>
                        <div class="btn-group">
                            ...
                            <a class="btn btn-primary d-flex justify-content-center align-items-center" href=""
                                data-bs-toggle="modal" data-bs-target="#create_usage_unit"
                                style="width: 120px; height: 40px; border-radius: 4px;">
                                បញ្ចូលអង្គភាព
                            </a>
                        </div>
            ...
            {{-- Modal create --}}
            <div class="modal fade" id="create_usage_unit" tabindex="-1" aria-labelledby="importModalLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            <div class="d-flex justify-content-center align-items-center" style="height: 20vh;">
                                <form id="uploadForm" action="{{ route('usage_units.store') }}" method="POST"
                                    style="width:100%;">
                                    @csrf
                                    <div class="mb-3 text-center">
                                        <input type="text" name="title_usage_unit" id="title_usage_unit"
                                            class="form-control mb-2" style="height: 40px;" placeholder="ឈ្មោះអង្គភាព"
                                            required>
                                        <input type="text" name="location_number" id="location_number"
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    ...
                    @forelse ($usageUnits as $index => $usageUnit)
                        <tr>
                            <td style="border: 1px solid black; text-align: center">
                                {{ ($usageUnits->currentPage() - 1) * $usageUnits->perPage() + $loop->index + 1 }}
                            </td>
                            
                            <td style="border: 1px solid black; text-align: start">
                                {{ $usageUnit->title_usage_unit }}</td>
                            <td style="border: 1px solid black; text-align: center">
                                {{ $usageUnit->location_number }}</td>
                            <td style="border: 1px solid black; text-align: center">
                                {{ $usageUnit->province_city }}
                            </td>
                            <td
                                style="border: 1px solid black; text-align: center; justify-content: center; width: 120px;">
                                <form id="delete-form-{{ $usageUnit->id }}"
                                    action="{{ route('usage_units.destroy', $usageUnit->id) }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                                <a class="btn btn-primary" href="{{ route('usage_units.edit', $usageUnit->id) }}">
                                    <i class="fas fa-edit" title="Edit"></i>
                                </a>
                                <button type="button" class="btn btn-danger"
                                    onclick="confirmDelete({{ $usageUnit->id }})">
            ...
            <div class="d-flex justify-content-between align-items-center mt-4">
                <div>
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            <li class="page-item{{ $usageUnits->onFirstPage() ? ' disabled' : '' }}">
                                <a class="page-link"
                                    href="{{ $usageUnits->previousPageUrl() }}&per_page={{ request('per_page') }}"
                                    aria-label="Previous">
                                    <span aria-hidden="true">&laquo;</span>
                                    <span class="sr-only">Previous</span>
                                </a>
                            </li>
                            @for ($i = 1; $i <= $usageUnits->lastPage(); $i++)
                                <li class="page-item{{ $usageUnits->currentPage() == $i ? ' active' : '' }}">
                                    <a class="page-link"
                                        href="{{ $usageUnits->url($i) }}&per_page={{ request('per_page') }}">{{ $i }}</a>
                                </li>
                            @endfor
                            <li class="page-item{{ !$usageUnits->hasMorePages() ? ' disabled' : '' }}">
                                <a class="page-link"
                                    href="{{ $usageUnits->nextPageUrl() }}&per_page={{ request('per_page') }}"
                                    aria-label="Next">
                ...
                <div>
                    <p class="text-muted">បង្ហាញ {{ $usageUnits->firstItem() }} ដល់ {{ $usageUnits->lastItem() }} នៃ
                        {{ $usageUnits->total() }} លទ្ធផល</p>
                </div>
            </div>
