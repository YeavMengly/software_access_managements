        ...
        <div class="result-total-table-container">
            ...
            <div class="d-flex justify-content-center align-items-center  ">
                <div class="card shadow-lg w-65" style="max-width: 900px;">
                    <!-- Centered Card Title -->
                    <h3 class="card-title text-center mt-4" style="font-weight: 500;">កែប្រែទិន្នន័យ</h3>

                    <div class="card-body px-5 py-4">
                        <form
                            action="{{ isset($loanMandate) ? route('loan-mandates.update', $loanMandate->id) : route('loan-mandates.store') }}"
                            method="POST" enctype="multipart/form-data">
                            @csrf
                            @if (isset($loanMandate))
                                @method('PUT')
                            @endif
                            ...
                            <div class="row mb-4">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="searchReportKey" class="form-control"
                                            placeholder="ស្វែងរកលេខកូដ..." onkeyup="filterReportKeys(event)"
                                        ...
                                        <select name="report_key" id="reportKeySelect" class="form-control" size="5"
                                            ...
                                            @foreach ($dataMandates as $dataMandate)
                                                <option value="{{ $dataMandate->id }}"
                                                    {{ isset($loanMandate) && $loanMandate->report_key == $dataMandate->id ? 'selected' : '' }}>
                                                    {{ $dataMandate->subAccountKey->sub_account_key }} >
                                ...
                                <!-- Column 2 -->
                                <div class="col-md-4">
                                    @foreach (['internal_increase' => 'កើនផ្ទៃក្នុង', 'unexpected_increase' => 'មិនបានគ្រោងទុក', 'additional_increase' => 'បំពេញបន្ថែម'] as $field => $label)
                                        <div class="form-group">
                                            <label for="{{ $field }}"
                                                class="form-label"><strong>{{ $label }}:</strong></label>
                                            <input type="number" name="{{ $field }}" id="{{ $field }}"
                                                class="form-control @error($field) is-invalid @enderror" min="0"
                                                oninput="formatNumber(this)"
                                                value="{{ old($field, $loanMandate->$field ?? 0) }}">
                                            @error($field)
                                ...
                                <!-- Column 3 -->
                                <div class="col-md-4">
                                    @foreach (['decrease' => 'ថយ', 'editorial' => 'វិចារណកម្ម'] as $field => $label)
                                        <div class="form-group">
                                            <label for="{{ $field }}"
                                                class="form-label"><strong>{{ $label }}:</strong></label>
                                            <input type="number" name="{{ $field }}" id="{{ $field }}"
                            ...
                            <div class="row">
                                <div class="col-12 text-center">
                                    ...
                                    <button type="submit" class="btn btn-primary ml-3">
                                        <i
                                            class="fas fa-save"></i>&nbsp;&nbsp;{{ isset($loanMandate) ? 'ធ្វើបច្ចុប្បន្នភាព' : 'រក្សាទុក' }}
                                    </button>
                                </div>
                            </div>
                        </form>
